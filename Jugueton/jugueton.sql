...
create table if not exists categorias_producto( #M
...
create table if not exists clasificaciones_producto( #M
...
create table if not exists productos( #M
	...
	foreign key (id_categoria) references jugueton.categorias_producto(id_categoria) on update restrict on delete restrict,
	foreign key (id_clasificacion) references jugueton.clasificaciones_producto(id_clasificacion) on update restrict on delete restrict,
	foreign key (id_marca) references jugueton.marcas_producto(id_marca) on update restrict on delete restrict
);
...
create table if not exists tipos_contactos( #M
    id_tipos_contacto int(11) unsigned not null auto_increment primary key,
    tipo_contacto varchar(35) unique not null
);
...
create table if not exists cuentas_compras( #I
    id_cuenta_compra int(11) unsigned not null auto_increment primary key,
	nombres_cuenta_compras varchar(100) not null,
	apellidos_cuenta_compras varchar(200) not null,
	usuario_cuenta_compras varchar(25) unique not null,
	contra_hash_cuenta_compras varchar(200) not null,
	creacion_cuenta_compras date not null,
	ultimo_acceso timestamp not null,
	id_dui int(11) unsigned,
	index (id_dui),
	foreign key (id_dui) references jugueton.duis(id_dui) on update restrict on delete restrict
);

create table if not exists contactos_cuentas_compras( #MV
	id_contacto_cuenta_compra int(11) unsigned not null auto_increment primary key,
	id_cuenta_compra int(11) unsigned not null,
	id_tipos_contacto int(11) unsigned not null,
	valor_contacto varchar(250) not null,
	creacion_cuenta_compras date not null,
	ultimo_acceso_cuenta_compras date not null,
	es_primario enum('false', 'true') default 'true' not null,
	index(id_tipos_contacto, id_cuenta_compra),
	foreign key (id_tipos_contacto) references jugueton.tipos_contactos(id_tipos_contacto) on update restrict on delete restrict,
	foreign key (id_cuenta_compra) references jugueton.cuentas_compras(id_cuenta_compra) on update restrict on delete restrict
);
...
create table if not exists descuentos( #MV
	id_descuento_porcentual int(11) unsigned not null auto_increment primary key,
...
create table if not exists categorizacion_juegos( #M
	id_categorizacion_juego int(11) unsigned not null auto_increment primary key,
	id_juego int(11) unsigned not null,
	id_categoria int(11) unsigned not null,
	index(id_juego, id_categoria),
	foreign key (id_juego) references jugueton.juegos(id_juego) on update restrict on delete restrict,
	foreign key (id_categoria) references jugueton.categorias_juegos(id_categoria_juego) on update restrict on delete restrict
);
...
create table if not exists cuentas( #M
	id_cuenta_mantenimiento int(11) unsigned not null auto_increment primary key,
...
create table if not exists permisos( #M1
    ...
    id_cuenta_mantenimiento int(11) unsigned not null,
	index(id_accion, id_pantalla, id_cuenta_mantenimiento),
	...
	foreign key (id_cuenta_mantenimiento) references jugueton.cuentas(id_cuenta_mantenimiento) on update restrict on delete restrict
);
...
create table if not exists listas( #I
	...
	es_carrito enum('false', 'true') default 'false' not null,
